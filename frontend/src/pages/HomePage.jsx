...
import { GitBranch, Map, MessageSquare, Zap, KeyRound, ChevronDown, ChevronUp } from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4">
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-xl">
        ...
        {/* PAT toggle — only show if not logged in via OAuth */}
        {!user && (
          <div className="mt-3">
            <button
              type="button"
            ...
            >
              ...
              {showPat ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
            </button>
            ...
            {showPat && (
              <div className="mt-2 flex flex-col gap-1.5">
                <input
                  ...
                  className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-brand-500 placeholder-gray-600 font-mono"
                />
                {pat && !patInput && (
                  <p className="text-xs text-green-500 flex items-center gap-1">
                    <KeyRound className="w-3 h-3" /> Token active for this session
                    <button
                      type="button"
                      onClick={() => savePat("")}
                      className="ml-2 text-gray-500 hover:text-red-400 underline"
                    >
                      clear
                    </button>
                  </p>
                ...
                <p className="text-xs text-gray-600">
                  Generate at github.com → Settings → Developer Settings → Personal Access Tokens → select <code className="text-gray-500">repo</code> scope
                </p>
              </div>
            )}
          </div>
